<template>
    <loadinglayout v-if="isFetchingData"></loadinglayout>
    <div v-else>
        <!-- progress bar -->
        ...
        <createproplayout>
            ...
            <template v-slot:button>
                ...
                <div class="btn-container" v-if="step == 11"> <button class="button-onward f-btn" @click.prevent="submitForm">Save changes <smbtnloading v-if="loading"></smbtnloading> </button></div>
            </template>
...
export default {
    ...
    methods: {
        ...
        async submitForm() {
            if (this.step == 11) {
                this.loading = true
                await this.$axios.patch(`/place/${this.$route.params.id}`, this.form)
                .then(result => {
                    console.log(result);
                    this.loading = false
        ...
        async getProperty() {
            await this.$axios.get(`/placetoedit/${this.$route.params.id}`, this.form)
            .then(result => {
                let newForm = result.data
                this.form = newForm
                console.log(this.form);
                return this.isFetchingData = false
            })
            .catch(e => {
                this.loading = false
